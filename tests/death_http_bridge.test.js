...
describe("death_http_bridge", () => {
    ...
    test("emits a kill event when the kills stat increments", () => {
        const harness = createHarness({ kills: 2 });
        harness.settle();
        harness.setKills(3);
        harness.runNextPoll();
    ...
    test("emits an assist event when the assists stat increments", () => {
        const harness = createHarness({ assists: 0 });
        harness.settle();
        harness.setAssists(1);
        harness.runNextPoll();
    ...
    test("does not emit kill or assist on load even when stats already start above zero", () => {
        const harness = createHarness({ kills: 4, assists: 2 });
        harness.runNextPoll();

        expect(actionable(harness)).toHaveLength(0);
    });

    test("does not emit kill or assist when stats stay flat or decrease", () => {
        const harness = createHarness({ kills: 3, assists: 1 });
        harness.setKills(3);
        harness.setAssists(1);
        harness.runNextPoll();
        harness.setKills(2);
        harness.setAssists(0);
        harness.runNextPoll();

        expect(actionable(harness)).toHaveLength(0);
    });

    test("kills and assists share the same global sequence as death and ability events", () => {
        const harness = createHarness({ kills: 0, assists: 0 });
        harness.settle();
        harness.setKills(1);
        harness.runNextPoll();
        harness.setDead(true);
        ...
        expect(actionable(harness).map(({ event, sequence }) => ({ event, sequence }))).toEqual([
            { event: "local_player_kill", sequence: 1 },
            { event: "local_player_death", sequence: 2 },
            { event: "local_player_assist", sequence: 3 },
        ]);
    });
